    ...
    <section className="space-y-8 rounded-4xl border border-white/60 bg-white/70 p-7 shadow-xl shadow-slate-900/5 backdrop-blur-2xl">
      ...
      <form onSubmit={handleSubmitCv}>
        <div className="space-y-4">
          ...
          <div className="flex items-center justify-between border-b border-gray-100 ">
            ...
            <button
              ...
              disabled={!isEditing}
              className="inline-flex cursor-pointer items-center gap-1.5 rounded-xl bg-indigo-950 px-3 py-1.5 text-[9px] font-black uppercase tracking-widest text-white shadow-sm shadow-indigo-900/25 transition hover:-translate-y-0.5 hover:bg-indigo-800 disabled:translate-y-0 disabled:opacity-40"
            >
              <Plus size={11} /> Add
            </button>
          </div>
          <div className="mt-4 space-y-4">
            {editor.experiences.map((item, index) => (
              <div
                key={`exp-${index}`}
                className="relative rounded-3xl border border-gray-100 bg-white/80 p-5 shadow-sm transition hover:border-indigo-100 hover:shadow-md"
              >
                <div className="mb-4 flex items-center justify-between">
                  ...
                  <button
                    ...
                    disabled={!isEditing}
                    className="inline-flex cursor-pointer items-center gap-1 rounded-lg border border-rose-100 bg-rose-50 px-2.5 py-1 text-[9px] font-black uppercase tracking-widest text-rose-500 transition hover:bg-rose-100 disabled:opacity-40"
                  >
                ...
                <div className="grid gap-2.5">
                  ...
                  <textarea
                    ...
                    className={`${subInputClass} min-h-22.5 resize-y`}
                  />
                </div>
              </div>
            ))}
        ...
        <div className="mt-4">
          ...
          <div className="space-y-4">
            {editor.educations.map((item, index) => (
              <div
                key={`edu-${index}`}
                className="relative rounded-3xl border border-gray-100 bg-white/80 p-5 shadow-sm transition hover:border-cyan-100 hover:shadow-md"
              >
                <div className="mb-4 flex items-center justify-between">
                  <div className="flex items-center gap-3">
                ...
                <div className="grid gap-2.5">
                  ...
                  <input
                    ...
                    value={item.startDate ?? ""}
                    onChange={(e) =>
                      onEducationChange(index, "startDate", e.target.value)
                    }
                    disabled={!isEditing}
                    className={subInputClass}
                  />
                  <input
                    ...
                    value={item.endDate ?? ""}
                    onChange={(e) =>
                      onEducationChange(index, "endDate", e.target.value)
                    }
                    disabled={!isEditing}
                    className={subInputClass}
                  />
                  <input
                    ...
                    value={item.gpa ?? ""}
                    onChange={(e) =>
                      onEducationChange(index, "gpa", e.target.value)
                    }
                    disabled={!isEditing}
                    placeholder="GPA"
                    className={subInputClass}
        ...
        <button
          ...
          className="mt-4 w-full cursor-pointer rounded-xl bg-white px-5 py-3 text-[11px] font-black uppercase tracking-widest text-indigo-950 shadow-lg transition-all hover:-translate-y-0.5 hover:bg-indigo-50 disabled:translate-y-0 disabled:opacity-50"
        >
          {loading.submit ? (
